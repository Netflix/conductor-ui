...
export default function ConductorTimeline({
  ...
  const collapsibleTasks = useMemo<Set<string>>(
    () =>
      new Set(
        data
          ?.filter((task) => collapseTaskTypes.includes(typeof task.taskType))
  ...
  const collapsedData = useMemo<Series[]>(() => {
    ...
    initialData.forEach((task, idx) => {
      ...
      if (
        !parentTaskRefName ||
        !collapseTaskTypes.includes(typeof taskTypeMap.get(parentTaskRefName))
      ) {
        data.push(task);
        ...
        .filter((task) => {
          ...
          if (
            ...
            !collapseTaskTypes.includes(
              typeof taskTypeMap.get(parentTaskRefName),
            )
          ) {
            return true;
          }
