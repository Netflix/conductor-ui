...
import { dagDoWhileDefOnly, dagDoWhileSuccess, dagDoWhileUnexecuted, dagDoWhileFailure, dagDoWhileRetries } from "../../src/utils/test/dagDoWhile";
...
describe('Do While', () => {
  describe("Success", () => {
    ...
    it("Placeholder is a stack of cards and COMPLETED", () => {
      cy.mount(<WorkflowGraph dag={dag} executionMode={true} />);
      cy.get("#loop_task_LOOP_CHILDREN_PLACEHOLDER").should('have.class', "type-LOOP_CHILDREN_PLACEHOLDER");
      cy.get("#loop_task_LOOP_CHILDREN_PLACEHOLDER").should('have.class', "status_COMPLETED");
    });

    it("Placeholder has correct count", () => {
      cy.mount(<WorkflowGraph dag={dag} executionMode={true} />);
      cy.get("#loop_task_LOOP_CHILDREN_PLACEHOLDER").should('contain.text', "10 of 10 tasks succeeded");
    });
    ...
    it("All edges executed and not dimmed", () => {
      cy.mount(<WorkflowGraph dag={dag} executionMode={true} />);
      cy.get(".edgePath").should('have.length', 5)
    ...
    it("Click on placeholder selects first task of first iteration", () => {
      ...
      cy.get('@onClickSpy').should('have.been.calledWith', { ref: "loop_task_child0__0" });;
    });
    ...
    it("Click on top bar selects DO_WHILE", () => {
      ...
      cy.get('@onClickSpy').should('have.been.calledWith', { ref: "loop_task" });;
    });
  ...
  describe('Unexecuted', () => {
    ...
    it("Loop tasks listed individually and have no color", () => {
      cy.mount(<WorkflowGraph dag={dag} executionMode={false} />);
      cy.get("#loop_task_child0").should('not.have.class', "status_COMPLETED").should('not.have.class', 'dimmed');
      cy.get("#loop_task_child1").should('not.have.class', "status_COMPLETED").should('not.have.class', 'dimmed');
    });
  ...
  describe('Failure', () => {
    ...
    it("Placeholder is a stack of cards and FAILED", () => {
      cy.mount(<WorkflowGraph dag={dag} executionMode={true} />);
      cy.get("#loop_task_LOOP_CHILDREN_PLACEHOLDER").should('have.class', "type-LOOP_CHILDREN_PLACEHOLDER");
      cy.get("#loop_task_LOOP_CHILDREN_PLACEHOLDER").should('have.class', "status_FAILED");
    });

    it("Placeholder shows failure count", () => {
      cy.mount(<WorkflowGraph dag={dag} executionMode={true} />);
      cy.get("#loop_task_LOOP_CHILDREN_PLACEHOLDER").should("contain.text", "9 of 10 tasks succeeded");
    });
