...
import { dagStaticForkSuccess, dagStaticForkFailure, dagStaticForkRetries, dagStaticForkUnexecuted } from "../../src/utils/test/dagStaticFork";
...
describe('Static Fork', () => {
  describe("Success", () => {
    ...
    it("Children COMPLETED and have ref and name", () => {
      cy.mount(<WorkflowGraph dag={dag} executionMode={true} />);
      for (let i = 0; i < 4; i++) {
        cy.get(`#static_fork_task_${i}`).should('have.class', "status_COMPLETED");
        cy.get(`#static_fork_task_${i}`).should('contain.text', `static_fork_task_${i}`);
        cy.get(`#static_fork_task_${i}`).should('contain.text', `(static_fork_task_${i}_name)`);
      }
    });
  ...
  describe("Unexecuted", () => {
    ...
    it("Children have ref and name and dimmed", () => {
      cy.mount(<WorkflowGraph dag={dag} executionMode={true} />);
      for (let i = 0; i < 4; i++) {
        cy.get(`#static_fork_task_${i}`).should('not.have.class', "status_COMPLETED");
